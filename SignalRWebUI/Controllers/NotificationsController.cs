...
namespace SignalRWebUI.Controllers
{
	public class NotificationsController : Controller
	{
		...
		public async Task<IActionResult> CreateNotification(CreateNotificationDto createNotificationDto)
		{
			...
			var responseMessage = await client.PostAsync("https://localhost:7037/api/Notifications", stringContent);
			if (responseMessage.IsSuccessStatusCode)
			{
		...
		public async Task<IActionResult> UpdateNotification(UpdateNotificationDto updateNotificationDto)
		{
			...
			StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
			var responseMessage = await client.PutAsync("https://localhost:7037/api/Notifications/", stringContent);
			if (responseMessage.IsSuccessStatusCode)
			{
				return RedirectToAction("Index");
			}
		...
		{
			var client = _httpClientFactory.CreateClient();
			await client.GetAsync($"https://localhost:7037/api/Notifications/ChangeStatustoTrue/{id}");
			return RedirectToAction("Index");
		}
		public async Task<IActionResult> ChangeStatustoFalse(int id)
		{
			var client = _httpClientFactory.CreateClient();
			await client.GetAsync($"https://localhost:7037/api/Notifications/ChangeStatustoFalse/{id}");
			return RedirectToAction("Index");
		}	
	}
}
